...
var (
	host = "127.0.0.1"
	port = uint16(65534)
	circ = "pkg/bip32_derive_tweak_ec.mpcl"
)
...
func main() {
	...
	var buf []byte
	if *evaluator {
		buf, err = evaluator_fn("dummy_session_id", args[0])
		log.Println("evaluator result:", hex.EncodeToString(buf))
	} else {
		buf, err = garbler_fn("dummy_session_id", args[0], args[1], args[2])
...
func evaluator_fn(
	...
	if slices.Compare(peerInputSizes, oPeerInputSizes) != 0 {
		circ, err = loadCircuit("pkg/bip32_derive_tweak_ec.mpcl", params, inputSizes)
		if err != nil {
			conn.Close()
			return nil, errors.Wrap(err, "in evaluator_fn()")
		}
...
func garbler_fn(
	...
	circ, err := loadCircuit(circ, params, inputSizes)
	if err != nil {
		return nil, errors.Wrap(err, "in garbler_fn()")
	}
...
func c_evaluator_fn(sid *C.char, ui *C.char, result_ptr **C.uchar, result_len *C.int) C.int {
	// Convert C strings to Go strings
	goSid := C.GoString(sid)
	goUi := C.GoString(ui)

	// Call the Go evaluator function
	buf, err := evaluator_fn(goSid, goUi)
	if err != nil {
...
//
//export c_garbler_fn
func c_garbler_fn(session_id *C.char, ui *C.char, cc *C.char, cnum *C.char, result_ptr **C.uchar, result_len *C.int) C.int {
	// Convert C strings to Go strings
	...
	buf, err := garbler_fn(goSessionId, goUi, goCc, goCnum)
	if err != nil {
