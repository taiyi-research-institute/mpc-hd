...
extern "C" {
    ...
    fn c_evaluator_fn(
        circ_file: *const c_char,
        sid: *const c_char,
        ui: *const c_char,
        ret_buf: *mut *mut c_uchar,
        ret_len: *mut c_int,
    ) -> c_int;
...
pub fn evaluator(circ_file: &str, session_id: &str, ui: &str) -> Result<Vec<u8>, String> {
    let c_circ_file = CString::new(circ_file).map_err(|e| e.to_string())?;
...
fn main() {
    ...
    let circ_file = std::env::var("MPC_circ_file")
        .unwrap_or_else(|_| "./apps/garbled/circ_file".to_string());

    println!("Using circuit directory: {}\n", circ_file);

    // Test evaluator
    println!("Testing evaluator function...");
    match evaluator(&circ_file, "test_session_1", "0x1919810") {
        Ok(result) => {
